  ...
  useEffect(() => {
    const fetchPackageData = async () => {
      try {
        ...
        if (response) {
          const formattedTags = response.tags ? response.tags.join(",") : "";

          setPackageData({
            ...response,
            tags: formattedTags, // Update the tags field
          });
          //   setImagePreview(URL.createObjectURL(response.img)); // Assuming img is a file or URL
          setImagePreview(
            URL.createObjectURL(await UploadFiles.getFile(response?.img))
          ); // Assuming img is a file or URL
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(packageData).forEach((key) => {
      if (key === "hotels" || key === "highlights") {
        formData.append(key, JSON.stringify(packageData[key])); // Append these fields as JSON
      } else {
        formData.append(key, packageData[key]); // Append other fields directly
      }
    });

    PackageApi.editPackage(formData, id);
    navigate("/manage-package");
  };

  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      <h1 className="text-2xl font-bold text-blue-600 mb-4">Edit Package</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <input
            type="checkbox"
            checked={packageData.isUmrahPlus}
            onChange={(e) => {
